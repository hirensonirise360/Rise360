...
import { ArrowRight, TrendingUp, GitMerge, Calculator, Shield, Globe } from "lucide-react";
...
export async function generateMetadata({
  ...
  if (!countryConfig) {
    return { title: "Services | RISE360 Global" };
  }

  return {
    title: `Services in ${countryConfig.name} | Global Expansion & Financial Advisory`,
    description: `Tailored advisory services for ${countryConfig.name}: Market Entry, Cross-Border M&A, Financial Outsourcing, and Regulatory Compliance.`,
    alternates: { canonical: `https://${countryConfig.domain}/services` },
  };
}
      ...
      <section className="relative pt-32 pb-20 bg-gradient-to-b from-blue-50/70 via-slate-50/50 to-white overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="flex items-center gap-2 mb-6">
            ...
            <span className="text-slate-900 text-sm font-bold">Services ({countryConfig.name})</span>
          </div>
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-blue-100 text-[#0052CC] rounded-full text-xs font-bold mb-4 border border-blue-200">
            <Globe size={14} />
            <span>RISE360 {countryConfig.name} Advisory Services</span>
          </div>

          <h1 className="font-display text-5xl lg:text-7xl font-bold text-[#012269] mb-6 leading-tight">
            Services built for <span className="italic gradient-text-dark">{countryConfig.name} Growth</span>
          </h1>
          <p className="text-slate-600 text-xl max-w-2xl leading-relaxed font-normal">
            Integrated, end-to-end consulting services tailored specifically for operations, compliance, and expansion in {countryConfig.name}.
          </p>
        </div>
      </section>
            ...
            {services.map(({ icon: Icon, title, slug, tagline, description, bullets, color, featured }) => (
              <div key={slug} className={`relative group rounded-3xl overflow-hidden flex flex-col ${featured ? "ring-2 ring-[#4DA6FF]/60 shadow-xl" : ""}`}>
                ...
                <div className={`bg-gradient-to-br ${color} p-8 flex flex-col flex-1`}>
                  ...
                  <Link href={`${countryPrefix}/services/${slug}`} className="inline-flex items-center gap-2 px-5 py-2.5 bg-white/15 hover:bg-white/25 text-white font-semibold text-sm rounded-xl transition-all border border-white/20 hover:border-white/40 w-fit group/btn">
